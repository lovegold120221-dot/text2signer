...
describe('LanguageSelectorComponent', () => {
  ...
  it('should keep automatic detection selected on initialization', () => {
    const autoFixture = TestBed.createComponent(LanguageSelectorComponent);
    const autoComponent = autoFixture.componentInstance;
    autoComponent.translationKey = 'languages';
    autoComponent.languages = ['en', 'fr', 'de'];
    autoComponent.hasLanguageDetection = true;
    autoComponent.language = null;

    autoFixture.detectChanges();

    expect(autoComponent.language).toBeNull();
    expect(autoComponent.selectedIndex).toBe(0);
  });

  it('should display dynamically detected three-letter source languages', () => {
    expect(component.langName('ilo')).toBe('Iloko');
  });
