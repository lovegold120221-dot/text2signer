...
export class SpokenLanguageInputComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    this.translate$
      .pipe(
        tap(({spokenLanguage, detectedLanguage}) => {
          ...
          this.spokenLanguage = spokenLanguage ?? detectedLanguage ?? globalThis.navigator?.language ?? 'en';
        }),
    ...
    this.text$
      .pipe(
        tap(text => this.text.setValue(text, {emitEvent: false})),
        takeUntil(this.ngUnsubscribe)
      )
      .subscribe();
  }
  ...
  receiveSpeechTranscript(update: SpeechTranscriptUpdate) {
    this.store.dispatch(new ReceiveSpeechTranscript(update));
  }
