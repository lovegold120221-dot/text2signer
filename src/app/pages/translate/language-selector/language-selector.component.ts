...
export class LanguageSelectorComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    if (!this.language && !this.hasLanguageDetection) {
    ...
    this.store
      ...
      .pipe(
        tap(detectedLanguage => {
          this.detectedLanguage = detectedLanguage;
          if (detectedLanguage && !this.langNames[detectedLanguage]) {
            this.langNames[detectedLanguage] = this.langName(detectedLanguage);
          }
        }),
        takeUntil(this.ngUnsubscribe)
      )
  ...
  langName(lang: string): string {
    if (this.displayNames && /^[a-z]{2,3}(-[a-z]{2})?$/i.test(lang)) {
